...
class EngineTest {
    @Test
    void tick_advancesWorldAndFiresEvent() throws Exception {
        Edt.run(() -> {
    ...
    @Test
    void enqueue_appliesCommandOnNextTick() throws Exception {
        Edt.run(() -> {
            Engine engine = new Engine(WorldGenerator.generate(1L));
    ...
    @Test
    void selectionChange_firesEvent() throws Exception {
        Edt.run(() -> {
            Engine engine = new Engine(WorldGenerator.generate(1L));
            List<EngineEvent> received = new ArrayList<>();
            engine.addListener(received::add);
